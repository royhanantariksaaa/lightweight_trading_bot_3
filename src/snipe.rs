...
pub fn find_last_minute_5m_snipes(settings: &Settings, markets: &[MarketSnapshot]) -> Vec<SnipeSignal> {
    let mut signals = markets
        ...
        .filter(|market| market.volume >= settings.snipe_min_volume_usd || market.liquidity >= settings.snipe_min_liquidity_usd)
        .flat_map(|market| market.outcomes.iter().filter_map(move |outcome| score_outcome(settings, market, outcome)))
        .collect::<Vec<_>>();

    signals.sort_by(|a, b| b.expected_edge.partial_cmp(&a.expected_edge).unwrap_or(std::cmp::Ordering::Equal));
    signals.truncate(settings.snipe_max_signals);
    signals
}

fn score_outcome(settings: &Settings, market: &MarketSnapshot, outcome: &OutcomeSnapshot) -> Option<SnipeSignal> {
    ...
    let time_pressure = 1.0 - (market.seconds_to_expiry as f64 / settings.snipe_window_seconds as f64).clamp(0.0, 1.0);
    ...
    let expected_edge = 0.45 * time_pressure + 0.35 * liquidity_quality + 0.20 * price_extremity - implied_prob;
