...
pub fn evaluate_strategy(settings: &Settings, state: &BotState, ctx: &StrategyContext) -> Decision {
    if ctx.quote_age_ms > settings.max_quote_age_ms {
        return Decision::Observe {
            reason: format!("stale quote age {}ms", ctx.quote_age_ms),
        };
    }
    ...
    if let Some(exit_at) = state.recent_exit_ms(&ctx.market_slug, &ctx.outcome) {
        ...
        if elapsed < settings.reentry_cooldown_ms {
            return Decision::Observe {
                reason: format!(
                    "same-side reentry cooldown active: {}ms remaining",
                    settings.reentry_cooldown_ms - elapsed
                ),
            };
        }
    }
    ...
    if state.bot_owns_position(&ctx.market_slug, &ctx.outcome) {
        ...
        if counts.exit_ticks < settings.exit_confirmation_ticks {
            return Decision::Observe {
                reason: format!(
                    "exit signal waiting for confirmation {}/{}",
                    counts.exit_ticks, settings.exit_confirmation_ticks
                ),
            };
        }
    ...
    if counts.entry_ticks < settings.entry_confirmation_ticks {
        return Decision::Observe {
            reason: format!(
                "entry signal waiting for confirmation {}/{} score={:.3}",
                counts.entry_ticks, settings.entry_confirmation_ticks, score
            ),
        };
    }
    ...
    if score < 0.75 {
        return Decision::Observe {
            reason: format!("score below entry threshold: {:.3}", score),
        };
    }
    ...
    if limit_price <= 0.0 || limit_price >= ctx.best_ask {
        return Decision::Observe {
            reason: format!("reservation bid {:.4} would cross/invalid", limit_price),
        };
    }
...
fn entry_score(ctx: &StrategyContext) -> f64 {
    ...
    let spread_quality = if spread <= 0.03 {
        1.0
    } else {
        (0.06 - spread).max(0.0) / 0.03
    };
...
fn reservation_bid(settings: &Settings, ctx: &StrategyContext) -> f64 {
    ...
    let inventory_penalty =
        ctx.inventory_shares * gamma * ctx.volatility.powi(2) * ctx.seconds_to_expiry;
